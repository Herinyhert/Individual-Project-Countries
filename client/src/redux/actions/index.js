...
export function getNameCountries(name){
    return async function(dispatch){
        try {
            let json = await axios.get("https://individual-project-countries-production.up.railway.app/countries?name=" + name,{});
            return dispatch({
...
export function getAllActivities(){
    return async (dispatch) => {
        var json = await axios.get('https://individual-project-countries-production.up.railway.app/activities')
       return dispatch({ type: GET_ALL_ACTIVITIES, payload: json.data})}
}

export function datailCountries(payload){
    return async function(dispatch){
        var json= await axios.get(`https://individual-project-countries-production.up.railway.app/countries/${payload}`);
        return dispatch({ type: DETAIL_COUNTRIES, payload: json.data })}
}

export function createActivity(payload){
    return async function(){
        let json = await axios.post("https://individual-project-countries-production.up.railway.app/activities", payload);
